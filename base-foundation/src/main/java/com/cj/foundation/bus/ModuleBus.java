...
import static com.cj.annotation.KeyKt.MODULE_EVENT_FILE_PRE;
...
public class ModuleBus {
    ...
    public void init(Context context) {
        ...
        try {
            ...
            for (String file : fileList) {
                ...
                if (fileName.startsWith(MODULE_EVENT_FILE_PRE) && fileName.endsWith(".json")) {
                    //解析json配置文件
    ...
    private ModuleEventCenterEntity parseModuleInfoByJson(String jsonString) throws JSONException, ParseException {
        ...
        return new ModuleEventCenterEntity(
    ...
    public synchronized <T> T of(Class<T> clz) {
        ...
        for (ModuleEventCenterEntity entity : eventCenterEntityList) {
            String intfName = clz.getSimpleName();
            if (!TextUtils.isEmpty(intfName)) {
                String[] splits = intfName.split("_");
        ...
        if (!TextUtils.isEmpty(moduleName)) {
            //动态生成的接口名
            //接口名：com.cj.bus.Gen_{moduleName}_Interface
            String interfaceName = "com.cj.bus.Gen_" + moduleName + "_Interface";
